...
namespace softaware.Authentication.Hmac.AspNetCore
{
    internal class HmacAuthenticationHandler : AuthenticationHandler<HmacAuthenticationSchemeOptions>
    {
        ...
        private async Task<ValidationResult> ValidateAsync(HttpRequest request)
        {
            ...
            {
                // Logger.LogDebug($"Authorization header =========== {header.ToString()}");
                var authenticationHeader = AuthenticationHeaderValue.Parse(header);
                // Logger.LogDebug($"Authentication header ======= {authenticationHeader.Scheme}");
                // Logger.LogDebug($"Options ====> {this.Options.AuthenticationScheme}");
                if (this.Options.AuthenticationScheme.Equals(authenticationHeader.Scheme, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // Logger.LogDebug($"raw authentication header =====> {rawAuthenticationHeader}");
                    ...
                    if (authenticationHeaderArray != null)
                    {
                        ...
                        // Logger.LogDebug(string.Format("appId={0} | sign={1} | nonce={2} | time={3}", appId, incomingBase64Signature, nonce, requestTimeStamp));
        ...
        {
            // Logger.LogDebug($"Check request validaty ========");
            ...
            {
                // Logger.LogDebug("............. Authorization Provider NOT Found!!");
                return false;
            }

            if (this.IsReplayRequest(nonce, requestTimeStamp))
            {
                // Logger.LogDebug("............. Is replay request ..............");
                return false;
            }
            ...
            // Logger.LogDebug($"Api KEY ......... {authorizationProviderResult.ApiKey}");
            ...
            using (var hmac = new HMACSHA256(apiKeyBytes))
            {
                ...
                var computedSignature = Convert.ToBase64String(signatureBytes);
                // Logger.LogDebug("Computed Signature ............ {0}", computedSignature);
                // Logger.LogDebug("Incoming signature ............ {0}", incomingBase64Signature);
